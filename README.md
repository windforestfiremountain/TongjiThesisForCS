# TongjiThesis (最后更新: 2026/01/08)
## 1. 总览
同济大学计算机硕士论文LaTeX模板修订版, 绝赞更新中!

本次修改的发起者：
* [SXN](https://github.com/windforestfiremountain)

特别致谢：
*  [WYQY](https://github.com/wyqy/TongjiThesis_Proto) 提供的2025版修订
...
- _(20260108) 初版 ： 在WYQY的2025版基础上进行了以下更新_
  - 注释了硕士论文不需要的内容, 如符号对照表示例、插图索引等;
  -  在 `thesis.tex` 主文档中添加了图、表、代码块、脚注(可带代码行)的宏包以及使用说明, 如 `\usepackage{tcolorbox}` 等，添加了预答辩版本单面打印的选项; 
  -  在附录部分给出了代码块图片示例，如大模型Prompt模板的样例;
  -  增加更详细的目录结构说明，见4.1. 目录结构
  -  在`chap01.tex` 中增加了论文写作经验和大模型GeminiPro辅助经验
...
> &nbsp;&nbsp;&nbsp;&nbsp;建议使用 VSCode + SumatraPDF, 可调用模板提供的配置文件直接编译和预览，配置完成后按`ctrl+S`可实现自动编译
...
如果你使用其他文献管理工具无法导出 bib 文件, 那么可以用Google Scholar导出 bibtex格式，直接复制到 refs.bib 文件中，引用时需要注意的是， bibtex 格式的引用需要在引用时添加 `\cite{}` 命令，而不是 `\parencite{}` 或 `\footcite{}` 等命令.
